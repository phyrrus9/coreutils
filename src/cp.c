...
void
print_phyrrus9_changes(void)
{
	fprintf(stdout, _("My changes include:\n"
			  "1. cp now defaults recursive, because most case uses are recursive. Running %s -r/R will disable recursive\n"),
			  program_name/*#1*/);
	exit(1);
}
...
usage (int status)
{
  fprintf(stderr, _("UNIX CP command, modified by phyrrus9 <phyrrus9@gmail.com>. Run %s --phyrrus9 to see my changes\n"),
			program_name);
...
"), stdout);
      fputs (_("\
  -R, -r, --recursive          disable copy directories recursively bit\n\
...
main (int argc, char **argv)
{
  ...
  while ((c = getopt_long (argc, argv, "abdfHilLnprst:uvxPRS:T",
	...
	case 'g':
	  print_phyrrus9_changes();
	  break;
        case 'a':
          /* Like -dR --preserve=all with reduced failure diagnostics.  */
	  x.recursive = true; //default to recurse (so we don't need to always cp -r)
